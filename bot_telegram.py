...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        file_bytes = await file_obj.download_as_bytearray()
        logger.info(f"File berhasil diunduh dengan ukuran {len(file_bytes)} bytes")
       
        imported = 0
       
        if filename.endswith((".xlsx", ".xls", ".csv")):
            try:
                # Gunakan BytesIO untuk membaca file dari memory
            ...
            try:
                logger.info(f"Berhasil membaca file gambar dengan ukuran {len(file_bytes)} bytes")
...
def main():
    ...
    try:
        ...
        # URL webhook dari environment variable
        WEBHOOK_URL = os.getenv("RAILWAY_PUBLIC_URL")
        if not WEBHOOK_URL:
            logger.error("RAILWAY_PUBLIC_URL tidak ditemukan di environment variables")
            return
        ...
        logger.info(f"Bot berjalan dengan webhook di {WEBHOOK_URL}")
        app.run_webhook(
            listen="0.0.0.0",
            port=PORT,
